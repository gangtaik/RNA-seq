...
pdir=/data/Epsilon_data8/parkji7/public_breast/breast.${geo}/fastq
...
#               	echo ${i}
#               	
#		/data3/gangtl95/BRCA/sratoolkit.3.0.0-ubuntu64/bin/prefetch ${i}
#
#		cd ${i}
#
#		mv ${i}.sra ../
...
#
#               	
#		/data3/gangtl95/BRCA/sratoolkit.3.0.0-ubuntu64/bin/vdb-validate ${i} 2> ./log/${i}.validation_out
#               	
...
#	echo ${j}
#	
#	/data3/gangtl95/BRCA/sratoolkit.3.0.0-ubuntu64/bin/fasterq-dump ../sra/${j}
#
#done
#
#cd ${sra}
...
#
#cp ${dir}combine.paired.py ${dir}breast.${geo}/
#		
#python3 ../combine.paired.py -i ${fastq} -o ${fastq} -T ../SraRunTable.txt
#
#
#cp ${dir}combine.single.py ${dir}breast.${geo}/
#
#python3 ../combine.single.py -i ${fastq} -o ${fastq} -T ../SraRunTable.txt
#
#
#
...
ls |grep fastq |awk -F '_' '{print $1}' |uniq > ${fastq}/tmp.list

cat ${fastq}/tmp.list |sed -e 's/.fastq//g' > ${fastq}/srr.list
...
do
	...
	then
		...
		echo -e ${sname}_1.fastq >> ${fastq}/base.p.1.txt 2>&1 

		awk 'BEGIN{sum=0;}{if(NR%4==2){sum+=length($0);}}END{print sum;}' ${sname}_1.fastq >> ${fastq}/base.p.2.txt 2>&1
	
		echo -e ${sname}_2.fastq >> ${fastq}/base.p.3.txt 2>&1

		awk 'BEGIN{sum=0;}{if(NR%4==2){sum+=length($0);}}END{print sum;}' ${sname}_2.fastq >> ${fastq}/base.p.4.txt 2>&1


	else
		echo "Library layouts is Single"

		echo -e ${sname}.fastq >> ${fastq}/base.s.1.txt 2>&1

		awk 'BEGIN{sum=0;}{if(NR%4==2){sum+=length($0);}}END{print sum;}' ${sname}.fastq >> ${fastq}/base.s.2.txt  2>&1
...
do
	...
	then
		...
		${aligner}/hisat2 \
			-p 4 \
                        -x ${genome}/genome -1 ${pdir}/${sname_1} \
                        -2 ${pdir}/${sname_2} \
